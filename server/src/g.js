...
const STORE = {
    ...
    post_tag: fse.readJSONSync(path.resolve(__dirname, "../database/post_tag.json"))
};
...
function generateHtml(){
 ...
 for (let index = 0; index < page; index++) {
     ...
     App_Page.postList = postIds.map(item => {
 ...
 console.log("create html finish...");
}
...
function generateSitemap(){
    ...
    console.log("create sitemap finish...");
}


function generate() {
    GenerateHtml.init();
    GenerateSitemap.init();
    setImmediate(() => {
        publish();
    }, 0);
}
